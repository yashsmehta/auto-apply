"""Enhanced Claude MCP integration with formatting helpers for web UI"""
import subprocess
import json
import os
import time
...
from .utils import safe_json_parse, chunk_text, create_error_response, url_cache


class ClaudeMCPError(Exception):
    """Custom exception for Claude MCP errors"""
...
class ClaudeMCP:
    """Enhanced wrapper for Claude MCP tool with web UI support and formatting utilities"""
    
    def __init__(self, work_folder: Optional[str] = None, timeout: int = 60, use_cache: bool = True):
        self.work_folder = work_folder or os.getcwd()
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get Claude MCP usage statistics"""
    ...
    def call_claude(self, prompt: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Call Claude MCP and return structured response for web UI
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                error_msg = result.stderr.strip() or "Unknown Claude MCP error"
                
                raise ClaudeMCPError(
                    f"Claude MCP failed: {error_msg}",
            ...
            raise ClaudeMCPError(
                f"Claude MCP timed out after {self.timeout} seconds",
            ...
            raise ClaudeMCPError(
                f"Unexpected error calling Claude MCP: {str(e)}",
    ...
    # Formatting methods from claude_helpers.py
    ...
    def _get_user_friendly_error(error_msg: str) -> str:
        ...
        elif "claude cli not found" in error_msg.lower():
            return "The AI assistant is not properly configured. Please contact support."
        elif "non-zero exit code" in error_msg.lower():
    ...
    def format_questions_for_display(questions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx, q in enumerate(questions):
            ...
            formatted_q["ui_type"] = ClaudeMCP._map_to_ui_type(formatted_q["type"])
            formatted_q["validation_rules"] = ClaudeMCP._get_validation_rules(formatted_q)
    ...
    def format_answers_for_review(answers: List[Dict[str, Any]], include_confidence: bool = True) -> Dict[str, Any]:
        ...
        for idx, ans in enumerate(answers):
            ...
            formatted_ans = {
                ...
                "editable": True,
                "char_count": len(ans.get("answer", ""))
            }
            
            if include_confidence:
                formatted_ans["confidence"] = confidence
                formatted_ans["confidence_indicator"] = ClaudeMCP._get_confidence_indicator(confidence)
                
            if ans.get("notes"):
                formatted_ans["notes"] = ans["notes"]
                
            formatted["answers"].append(formatted_ans)
        
        formatted["confidence_summary"] = {
            "percentage_high": round((formatted["high_confidence"] / len(answers)) * 100) if answers else 0,
            "needs_review": formatted["low_confidence"] > 0
        }
        
        return formatted
    
    @staticmethod
    def _get_confidence_indicator(confidence: str) -> Dict[str, str]:
        """Get visual indicators for confidence levels"""
        indicators = {
            "high": {
                "color": "#10b981",
                "icon": "check-circle",
            ...
            "low": {
                "color": "#ef4444",
                "icon": "x-circle", 
                "text": "Low confidence - manual input recommended"
            }
        }
        return indicators.get(confidence, indicators["medium"])
...
class ApplicationStateManager:
    ...
    def get_progress(self, session_id: str) -> Dict[str, Any]:
        """Get progress summary for web UI"""
    ...
    def _get_current_step(self, state: Dict[str, Any]) -> Optional[str]:
